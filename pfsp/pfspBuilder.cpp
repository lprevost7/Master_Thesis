...
#define PROBLEM_PFS_TPLS "PFSP_TPLS" /** NEW Parameter for Multiobjectf */
#define PROBLEM_PFS_PLS "PFSP_PLS" /** NEW Parameter for Multiobjectf */
#define PROBLEM_PFS_TPPLS "PFSP_TPPLS" /** NEW Parameter for Multiobjectf */
...
#define GA_TOURNAMENT "ga_tournament"
#define GA_OX "ga_ox"
#define GA_SWAPMUT "ga_swapmut"
...
char* problem_string;
char* problem_string1;
char* problem_string2;
...
std::string info_pfsp()
{
    ...
    oss << "PROBLEM               = "<<PROBLEM_PFS_WT<< " " <<PROBLEM_PFS_WE<< " " <<PROBLEM_PFS_TCT<< " " <<PROBLEM_PFS_MS << " " <<PROBLEM_PFS_TPLS
...
emili::GASelection* prs::PfspBuilder::buildGASelection()
{
    prs::incrementTabLevel();
    emili::GASelection* sel = nullptr;
    if(tm.checkToken(GA_TOURNAMENT))
    {
        printTab("GA tournament selection");
        int k = tm.getInteger();
        printTabPlusOne("k",k);
        sel = new emili::pfsp::PfspGATournamentSelection(k);
    }
    prs::decrementTabLevel();
    return sel;
}

emili::GACrossover* prs::PfspBuilder::buildGACrossover()
{
    prs::incrementTabLevel();
    emili::GACrossover* cr = nullptr;
    emili::pfsp::PermutationFlowShop* instance =(emili::pfsp::PermutationFlowShop*) gp.getInstance();
    if(tm.checkToken(GA_OX))
    {
        printTab("GA OX crossover");
        cr = new emili::pfsp::PfspGAOXCrossover(*instance);
    }
    prs::decrementTabLevel();
    return cr;
}

emili::GAMutation* prs::PfspBuilder::buildGAMutation()
{
    prs::incrementTabLevel();
    emili::GAMutation* mu = nullptr;
    emili::pfsp::PermutationFlowShop* instance =(emili::pfsp::PermutationFlowShop*) gp.getInstance();
    if(tm.checkToken(GA_SWAPMUT))
    {
        printTab("GA swap mutation");
        mu = new emili::pfsp::PfspGASwapMutation(*instance);
    }
    prs::decrementTabLevel();
    return mu;
}
...
{    
    ...
    else if(strcmp(t,PROBLEM_PFS_TPLS)==0)
    {
#ifdef ENABLE_SSE
        prs::printTab("SSE run path enabled");
#endif
        prs::printTab("Permutation Flow Shop Multiobjective TPLS");
        prob = new emili::pfsp::PFSP_TPLS(i);
    }
    else if(strcmp(t,PROBLEM_PFS_PLS)==0)
    {
#ifdef ENABLE_SSE
        prs::printTab("SSE run path enabled");
#endif
        prs::printTab("Permutation Flow Shop Multiobjective PLS");
        prob = new emili::pfsp::PFSP_PLS(i);
    }
    else if(strcmp(t,PROBLEM_PFS_TPPLS)==0)
    {
#ifdef ENABLE_SSE
        prs::printTab("SSE run path enabled");
#endif
        prs::printTab("Permutation Flow Shop Multiobjective TPPLS");
        prob = new emili::pfsp::PFSP_TPPLS(i);
    }
...
emili::Problem* prs::PfspBuilder::openInstance()
{
    ...
    std::cout << "NAME PROBLEM" << std::endl;
    std::cout << "Problem: " << problem_string << std::endl;
    ...
    if(ok)
     {
        ...
        //  /** New */
        //  if(dynamic_cast<emili::pfsp::PFSP_TPLS*> (instance))
        //  {
        //     std::cout << "Problem is a multiobjective problem" << std::endl;
        //  }
        //  else
        //  {
        //     std::cout << "Problem is a single objective problem" << std::endl;
        //  }
        ...
        if(dynamic_cast<emili::pfsp::PFSP_TPLS*> (instance))
        {
            std::cout << "Problem is a multiobjective problem" << std::endl;
            PfspInstance i;    
            problem_string1 = tm.nextToken();
            problem_string2 = tm.nextToken();
            bool ok;
            std::string pro(problem_string1);
            // std::string pro(problem_string2);
            std::string sdst("SDSTPFS");

            std::cout << "NAME PROBLEM" << std::endl;
            std::cout << "Problem: " << problem_string1 << std::endl;
            ...
            if(ok)
            {
                emili::pfsp::PermutationFlowShop* instance1 = loadProblem(problem_string1, i);
                emili::pfsp::PermutationFlowShop* instance2 = loadProblem(problem_string2, i);
...
/** New */
emili::Problem* prs::PfspBuilder::openInstance(emili::Problem* instance1, emili::Problem* instance2)
{
    PfspInstance i;    
    problem_string = tm.nextToken();
    bool ok;
    ...
    if(ok)
     {
         emili::pfsp::PermutationFlowShop* instance = loadProblem(problem_string, i);
        /** New */
        is_tpls = (std::strcmp(problem_string, PROBLEM_PFS_TPLS) == 0);
        is_pls = (std::strcmp(problem_string, PROBLEM_PFS_PLS) == 0);
        is_tppls = (std::strcmp(problem_string, PROBLEM_PFS_TPPLS) == 0);

        if(is_tpls || is_pls || is_tppls)
        {
            std::cout << "Problem is multiobjective" << std::endl;
            PfspInstance i;    
            problem_string1 = tm.nextToken();
            problem_string2 = tm.nextToken();
            bool ok;
            ...
            if(ok)
            {
                instance1 = loadProblem(problem_string1, i);
                instance2 = loadProblem(problem_string2, i);

                std::cout << typeid(*instance1).name() << std::endl;
                std::cout << typeid(*instance2).name() << std::endl;

                if(is_pls)
                {
                    return new emili::pfsp::PFSP_PLS(i, instance1, instance2);
                }
                else if (is_tppls)
                {
                    return new emili::pfsp::PFSP_TPPLS(i, instance1, instance2);
                }
                
                return new emili::pfsp::PFSP_TPLS(i, instance1, instance2);
...
bool isParsable(std::string &problem)
{
    ...
    /** NEW Parsable meth */
    else if(strcmp(problem.c_str(),PROBLEM_PFS_TPLS)==0)
    {
        return true;
    }else if(strcmp(problem.c_str(),PROBLEM_PFS_PLS)==0)
    {
        return true;
    }else if(strcmp(problem.c_str(),PROBLEM_PFS_TPPLS)==0)
    {
        return true;
...
// PFSP GA operator implementations
int emili::pfsp::PfspGATournamentSelection::select(const std::vector<emili::Solution*>& population)
{
    if(population.empty()) return 0;
    int bestIndex = 0;
    double bestValue = population[0]->getSolutionValue();
    int popsize = (int)population.size();
    for(int i = 0; i < k; ++i)
    {
        int idx = emili::generateRandomNumber() % popsize;
        double v = population[idx]->getSolutionValue();
        if(v < bestValue)
        {
            bestValue = v;
            bestIndex = idx;
...
emili::Solution* emili::pfsp::PfspGAOXCrossover::crossover(const emili::Solution& parent1, const emili::Solution& parent2)
{
    const emili::pfsp::PermutationFlowShopSolution* p1 = dynamic_cast<const emili::pfsp::PermutationFlowShopSolution*>(&parent1);
    const emili::pfsp::PermutationFlowShopSolution* p2 = dynamic_cast<const emili::pfsp::PermutationFlowShopSolution*>(&parent2);
    if(!p1 || !p2) return nullptr;

    std::vector<int> s1 =
        const_cast<emili::pfsp::PermutationFlowShopSolution*>(p1)->getJobSchedule();
    std::vector<int> s2 =
        const_cast<emili::pfsp::PermutationFlowShopSolution*>(p2)->getJobSchedule();
    // ---------------------------------------------------------

    int nj = instance.getNjobs();
    std::vector<int> child(nj+1, 0);
    int a = emili::generateRandomNumber() % nj + 1;
    int b = emili::generateRandomNumber() % nj + 1;
    if(a > b) std::swap(a,b);
    ...
    for(int i = 1; i <= nj; ++i)
    {
        int gene = s2[(b + i - 1) % nj + 1];
        bool present = false;
        for(int j = a; j <= b; ++j)
            if(child[j] == gene) { present = true; break; }
        if(!present)
        {
            child[pos] = gene;
            pos++;
    ...
    int val = instance.computeObjectiveFunction(child);
    return new emili::pfsp::PermutationFlowShopSolution(val, child);
}

void emili::pfsp::PfspGASwapMutation::mutate(emili::Solution& individual)
{
    emili::pfsp::PermutationFlowShopSolution* sol = dynamic_cast<emili::pfsp::PermutationFlowShopSolution*>(&individual);
    if(!sol) return;
    std::vector<int>& seq = sol->getJobSchedule();
    int nj = instance.getNjobs();
    if(nj < 2) return;
    int i = emili::generateRandomNumber() % nj + 1;
    int j = emili::generateRandomNumber() % nj + 1;
    while(j == i) j = emili::generateRandomNumber() % nj + 1;
    std::swap(seq[i], seq[j]);
    instance.evaluateSolution(*sol);
}
