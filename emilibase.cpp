...
{
    neighbh->reset();
    emili::Solution* current = nullptr;

    // if(this->getFirstStep())
    // {
    //     current = init->generateSolution();
    //     this->changeFirstStep();
    // }
    // else
    // {
    //     current = this->getTPLSPreviousBestSoFar();
    // }
...
/** New */
void emili::LocalSearch::setStep(int tpls_step)
{
    this->tpls_step = tpls_step;
}

void emili::LocalSearch::setAnyTime(bool anytime)
{
    this->anytime = anytime;
}

int emili::LocalSearch::getTPLSStep()
{
    return this->tpls_step;
}

bool emili::LocalSearch::getAnyTime()
{
    return this->anytime;
}

bool emili::LocalSearch::getFirstStep()
{
    return this->first_step;
}

void emili::LocalSearch::changeFirstStep()
{
    this->first_step = !this->first_step;
}

emili::Solution* emili::LocalSearch::getTPLSPreviousBestSoFar()
{
    return this->tpls_previousBestSoFar;
}

void emili::LocalSearch::changeTPLSPreviousBestSoFar(emili::Solution* newBest)
{
    // std::cout << "try to change best so far" << std::endl;
    // if(tpls_previousBestSoFar != nullptr)
    // {
    //     delete tpls_previousBestSoFar;
    // }

    // std::cout << "best so far changed" << std::endl;

    this->tpls_previousBestSoFar = newBest->clone();
}
...
void emili::GeneticAlgorithm::clearPopulation()
{
    for(std::vector<emili::Solution*>::iterator it = population.begin();
...
void emili::GeneticAlgorithm::initializePopulation()
{
    clearPopulation();
    ...
    for(int i = 0; i < populationSize; ++i)
    {
        ...
        if(i == 0)
        {
            // primer individuo: lo usamos como mejor
            setBest(s);
        }
        else if(*s < *bestSoFar)
        {
...
emili::GeneticAlgorithm::GeneticAlgorithm(InitialSolution& initial,
...
{
    clearPopulation();
    // Asumimos que estos operadores se crearon con new en el builder
    delete &selection;
    delete &crossover;
    delete &mutation;
...
emili::Solution* emili::GeneticAlgorithm::search()
{
    ...
    // Vector auxiliar para la nueva generación
    std::vector<emili::Solution*> offspring;
    offspring.reserve(populationSize);

    // Mejor de la población actual
    emili::Solution* iterationBest = population[0];
    for(int i = 1; i < populationSize; ++i)
    {
        if(*population[i] < *iterationBest)
        {
            iterationBest = population[i];
        }
    }
    setBest(iterationBest);

    do
    {
        // Limpiamos posibles hijos de la iteración anterior
        for(std::vector<emili::Solution*>::iterator it = offspring.begin();
            it != offspring.end(); ++it)
        {
            delete *it;
        }
        ...
        while((int)offspring.size() < populationSize)
        {
            int i1 = selection.select(population);
            int i2 = selection.select(population);

            if(i1 < 0) i1 = 0;
            if(i2 < 0) i2 = 0;
            if(i1 >= (int)population.size()) i1 = (int)population.size()-1;
            if(i2 >= (int)population.size()) i2 = (int)population.size()-1;

            emili::Solution* parent1 = population[i1];
            emili::Solution* parent2 = population[i2];
            emili::Solution* child = nullptr;

            float r = emili::generateRealRandomNumber();
            if(r < crossoverRate)
            {
                // Cruce (definido en una clase específica de VRP)
                child = crossover.crossover(*parent1, *parent2);
            }
            else
            {
                // Sin cruce: clon del padre 1
                child = parent1->clone();
            }
        ...
        // Mejor de la nueva generación (offspring)
        iterationBest = offspring[0];
        for(int i = 1; i < populationSize; ++i)
        {
            if(*offspring[i] < *iterationBest)
            {
                iterationBest = offspring[i];
            }
        }
        ...
        // Count one generation (for global iteration statistics)
        emili::iteration_increment();

    } while(!termcriterion->terminate(bestSoFar, iterationBest));

    // Liberamos población actual (no la necesitamos después)
    clearPopulation();

    // Devolvemos un clon de la mejor solución encontrada
    return bestSoFar->clone();
}

emili::Solution* emili::GeneticAlgorithm::search(emili::Solution* initial)
{
    // Por ahora ignoramos la solución inicial; podrías usarla para
    // sembrar la población en el futuro.
    (void)initial;
    return search();
}
