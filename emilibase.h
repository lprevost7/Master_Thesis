...
Solution* tpls_previousBestSoFar;
...
public:
    ...
    */
    ...
    /** New */
    virtual void setStep(int tpls_step);
    virtual void setAnyTime(bool anytime);
    ...
    bool getAnyTime();
    bool getFirstStep();
    void changeFirstStep();
    emili::Solution* getTPLSPreviousBestSoFar();
    void changeTPLSPreviousBestSoFar(emili::Solution* newBest);
    // emili::Solution* getInitialSolution();
...
/**
 * @brief GASelection
 * Operador de selección para algoritmos genéticos.
 * Devuelve el índice del individuo seleccionado en la población.
 */
class GASelection
{
public:
    virtual ~GASelection() { }
    virtual int select(const std::vector<emili::Solution*>& population) = 0;
};

/**
 * @brief GACrossover
 * Operador de cruce para algoritmos genéticos.
 * Debe crear y devolver un nuevo individuo hijo.
 */
class GACrossover
{
public:
    virtual ~GACrossover() { }
    virtual emili::Solution* crossover(const emili::Solution& parent1,
                                       const emili::Solution& parent2) = 0;
};

/**
 * @brief GAMutation
 * Operador de mutación para algoritmos genéticos.
 * Modifica in-place el individuo recibido.
 */
class GAMutation
{
public:
    virtual ~GAMutation() { }
    virtual void mutate(emili::Solution& individual) = 0;
};
...
/**
 * @brief GeneticAlgorithm
 * Implementación genérica de un algoritmo genético generacional.
 *
 * - No conoce la representación de la solución.
 * - Usa InitialSolution para crear la población inicial.
 * - Usa GASelection, GACrossover y GAMutation para la parte evolutiva.
 * - Usa Termination para decidir cuándo parar.
 */
class GeneticAlgorithm : public emili::LocalSearch
{
protected:
...
public:
    GeneticAlgorithm(InitialSolution& initial,
    ...
    // Versión con solución inicial: de momento la ignoramos o la podrías
    // usar para "sembrar" la población en el futuro
    virtual emili::Solution* search(emili::Solution* initial);
};
