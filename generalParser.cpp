...
#define RO "-ro"
#define IT "-it"
#define TPLSSTEP "-step"
#define TPLSANY "-tplsany"
#define RNDSEED "rnds"
#define PRINT_SOLUTION "ps"
#define DEFAULT_IT 0
#define DEFAULT_TPLSSTEP 100
#define GIT_COMMIT_NUMBER "3c2e6acc9512d608c2d7c2506c0faa79a32d1b6e"
/*Base Initials*/
...
/**
 * Extended time option scanner.
 * The original getTime() only recognizes -it or -ro if they appear exactly
 * at the current parsing position (i.e., immediately after all algorithm
 * components). In multiobjective runs users may append -step before -it,
 * leaving the token pointer sitting on -step when getTime() is invoked.
 * That caused the time option to be ignored and later phases to fall back
 * on hard–coded defaults (e.g., 100 seconds in plsPhase).
 * This helper scans the whole token list for -it / -ro regardless of order
 * without consuming the tokens or disturbing the parser state.
 */
static float scanTimeOption(prs::TokenManager& tm,int problemSize)
{
    int posIt = tm.seek(IT);
    int posRo = tm.seek(RO);
    float chosen = 0.0f;
    // Decide precedence based on earliest appearance among -it / -ro.
    if(posIt >= 0)
    {
        char* valTok = tm.tokenAt(posIt+1);
        if(valTok)
        {
            chosen = std::atof(valTok);
        }
    }
    if(posRo >= 0)
    {
        char* valTok = tm.tokenAt(posRo+1);
        if(valTok)
        {
            float d = std::atof(valTok);
            float computed = d * problemSize;
            // If -ro appears before -it or -it not present, prefer -ro.
            if(chosen == 0.0f || (posRo < posIt) || posIt < 0)
            {
                chosen = computed;
            }
        }
...
float getStep(prs::TokenManager& tm)
{
    if(tm.checkToken(TPLSSTEP))
    {
        int n = tm.getDecimal();
        std::ostringstream oss;
        oss << "Step : " << n;
    ...
    return DEFAULT_TPLSSTEP;
}

bool getTPLSAnyStep(prs::TokenManager& tm)
{
    if(tm.checkToken(TPLSANY))
    {
        std::ostringstream oss;
        oss << "Anytime step";
        //printTab(oss.str().c_str());
...
prs::Component prs::Builder::buildComponent(int type)
{
    ...
    case COMPONENT_GA_SELECTION: raw_pointer = buildGASelection();c.setType(COMPONENT_EMPTY);break;
    case COMPONENT_GA_CROSSOVER: raw_pointer = buildGACrossover();c.setType(COMPONENT_EMPTY);break;
    case COMPONENT_GA_MUTATION: raw_pointer = buildGAMutation(); c.setType(COMPONENT_EMPTY); break;
...
emili::LocalSearch* prs::GeneralParserE::parseParams()
{
    ...
    if(probBuilder != nullptr)
    {
        ...
        /** New */
        this->instance = probBuilder->openInstance(this->instance1, this->instance2);
        /** New */
        ...
        // Original positional parsing
        ls->setSearchTime(getTime(tm,ls->getInitialSolution().getProblem().problemSize()));
        // If no time captured (e.g., -it placed after -step) perform a global scan.
        if(ls->getSearchTime() <= 0.0f)
        {
            float scanned = scanTimeOption(tm, ls->getInitialSolution().getProblem().problemSize());
            if(scanned > 0.0f)
            {
                std::cout << "(order–independent) Run time secs : " << scanned << std::endl;
                ls->setSearchTime(scanned);
            }
        }
        ls->setStep(getStep(tm));
        ls->setAnyTime(getTPLSAnyStep(tm));
        emili::setRootAlgorithm(ls);
...
std::string prs::GeneralParserE::typeName(int type)
{
    ...
    case COMPONENT_GA_SELECTION: return std::string("GA selection");
    case COMPONENT_GA_CROSSOVER: return std::string("GA crossover");
    case COMPONENT_GA_MUTATION: return std::string("GA mutation");
    }
...
emili::LocalSearch* prs::EmBaseBuilder::buildAlgo()
{
    ...
    else if(tm.checkToken(ALG_GA))
    {
        ...
        double cr      = tm.getDecimal();   // crossoverRate
        double mr      = tm.getDecimal();   // mutationRate

        std::ostringstream oss;
        oss.str(""); oss << "population size = " << popSize;
        printTabPlusOne(oss.str().c_str());
        oss.str(""); oss << "crossover rate = " << cr;
        printTabPlusOne(oss.str().c_str());
        ...
        emili::GASelection* gaSel   = retrieveComponent(COMPONENT_GA_SELECTION).get<emili::GASelection>();
